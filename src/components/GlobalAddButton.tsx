...
export const GlobalAddButton: React.FC<GlobalAddButtonProps> = ({ 
  ...
  const actions = [
    ...
    { id: 'payment', label: 'Record Payment (+)', icon: DollarSign, color: 'bg-emerald-500', tab: 'invoices', permission: 'invoices' },
        ...
        {isOpen && (
          <>
            ...
            <motion.div 
              ...
              className="absolute bottom-20 right-0 w-72 bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 p-4 space-y-2 overflow-hidden"
            >
              <div className="px-4 py-2 border-b border-slate-50 dark:border-slate-800 mb-2">
                <h3 className="text-sm font-black text-slate-900 dark:text-slate-100 uppercase tracking-widest">Quick Actions</h3>
              </div>
              <div className="grid grid-cols-1 gap-1">
                {filteredActions.map((action) => (
                  <button
                    ...
                    className="flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all group text-left"
                  >
                    <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg", action.color)}>
                      <action.icon size={20} />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-bold text-slate-900 dark:text-slate-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{action.label}</p>
